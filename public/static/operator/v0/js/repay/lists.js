...
$(function () {
    layui.use(["laypage","layer"],function () {
       ...
       $.get("/operator/repay?p=1",function (re) {
           if(re.status == 200){
               laypage({
                   cont: 'pager-nav'
                   ...
                   ,jump: function(obj, first){
                       if(!first){
                           $.get("/operator/repay?p="+obj.curr,function (re) {
                                if(re.status == 200){
...
function setData(list) {
    ...
    $(list).each(function (index,ele) {
        ...
        content += "<tr>" +
                ...
                "<td>等本等息</td>" +
                '<td><a href="/operator/asset/'+ele.apply_id+'">查看</a></td>' +
                '<td><a href="/operator/asset/details/'+ele.apply_id+'">查看</a></td>' +
                "</tr>";
    });
    return content;
}
