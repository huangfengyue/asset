...
$(function () {
    layui.use(["layer","form"],function () {
        ...
        form.on("submit(submit)",function (data) {
            var file = 0;
            $("input[name='certFile[]']").each(function (index,ele) {
                if($(ele).val().length > 1){
                    file ++;
                }
            })
            if(file < 1){
                layer.msg("必须上传证件才能提交");return false;
            }
...
function fileInputAdd(object) {
    var c = '<div class="form-group">' +
        ...
        '<div class="col-sm-5">' +
        '<input type="file" name="certFile[]" class="form-control form-file" onchange="fileInputChange(this)">' +
