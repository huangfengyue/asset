...
$(function () {
   layui.use(["layer","form"],function () {
       ...
       form.on("submit(submit)",function (data) {
            $("input.require-type").each(function(index,ele){
                var type_id = $(ele).val();
                ...
                console.log(count);
                if(count < 1){
                    layer.msg("必须上传"+$(ele).attr("type-name"));
                    return false;
                }
            });
            if(count < 1){
...
function fileInputAdd(object) {
    var c = '<div class="form-group">' +
        ...
        '<div class="col-sm-5">' +
        '<input type="file" class="form-control form-file" name="type_id_'+$(object).attr("type-id")+'[]" onchange="fileInputChange(this)">' +
