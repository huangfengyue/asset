...
$(function () {
    layui.use(["laypage","layer"],function () {
       ...
       $.get("/agency/repay?p=1",function (re) {
           if(re.status == 200){
               laypage({
                   cont: 'pager-nav'
                   ...
                   ,jump: function(obj, first){
                       if(!first){
                           $.get("/agency/repay?p="+obj.curr,function (re) {
                                if(re.status == 200){
...
function SendForms ()
{
    $(function () {
        layui.use(["layer"],function () {
            ...
            var time2=$("#time2").val();
            var times=$("#times").val();
            window.location.href = "/agency/repay/repay_excel?p=1&time1="+time1+"&time2="+time2+"&times="+times;
            return false;
...
function SendForm ()
{
    $(function () {
        layui.use(["laypage","layer"],function () {
            ...
            var time1=$("#time1").val();
            var time2=$("#time2").val();
            $.post("/agency/repay?p=1&times="+times+"&time1="+time1+"&time2="+time2,function (re) {
                if(re.status == 200){
                    laypage({
                        cont: 'pager-nav'
                        ,pages: re.data.pager.totalPage
                        ,jump: function(obj, first){
                            if(!first){
                                $.post("/agency/repay?p="+obj.curr+"&times="+times+"&time1="+time1+"&time2="+time2,function (re) {
                                    if(re.status == 200){
                                        $("#data-table").find("tbody").html(setData(re.data.list))
                                    }
                                });
...
function setData(list) {
    ...
    $(list).each(function (index,ele) {
        content += "<tr>" +
            ...
            '<td><a href="/agency/asset/caiwu/'+ele.apply_id+'" style="color: black;">查看</a></td>' +
