...
Route::group(["prefix"=>"asset"],function (){
    ...
    Route::match(["post","get"],"/{apply_id}/check1","AssetController@check1")->where("apply_id",'^\d+$');   //风控一审
    Route::match(["post","get"],"/{apply_id}/sign","AssetController@sign")->where("apply_id",'^\d+$');   //风控一审签约
    Route::match(["post","get"],"check2/{get?}","AssetController@listByCheck2")->where("get",'^(\?\S*)|(\S{0})$');//二审列表
    Route::match(["post","get"],"/{apply_id}/check2","AssetController@check2")->where("apply_id",'^\d+$');;   //风控二审
