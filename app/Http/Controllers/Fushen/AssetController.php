...
//                die();
    ...
    public function updateApply(Request $request,$apply_id){
        if($request->ajax()){
            ...
            if($pass == "ignore"){
                $pass = AssetApply::$STATUS_RETRIAL_FAIL;
            }elseif ($pass == "back"){
                $pass = AssetApply::$STATUS_WAIT_FIRST_TRIAL;
            }else{
                $pass = AssetApply::$STATUS_WAIT_GENERATE;
            }
            if($applyInfo = $this->AssetService->getApplyDetailById($apply_id)){
                if(!in_array($applyInfo->status,[AssetApply::$STATUS_RETRIAL])){
                    return $this->responseJson(0,"该单状态异常，无法更改信息");
                }
                if($this->AssetService->applyFushen($apply_id,$pass,$remark,$this->userInfo->user_id)){
                    return $this->responseJson(200,"保存成功");
                }else{
    ...
    public function jlx(Request $request,$apply_id){
        ...
        if($a["success"]=="false"){
            return $this->error($a["note"],"/fushen/asset/$apply_id");
        }else{
    ...
    public function list_excel(Request $request){
        ...
        Excel::create('复审审件列表',function($excel) use ($cellDatas){
    ...
    public function ck2_excel(Request $request){
        ...
        Excel::create('复审面签列表',function($excel) use ($cellDatas){
            $excel->sheet('score', function($sheet) use ($cellDatas){
                $sheet->rows($cellDatas);
            });
        })->export('xls');
    ...
    public function repay_excel(Request $request){
        ...
        Excel::create('复审还款列表',function($excel) use ($cellDatas){
            $excel->sheet('score', function($sheet) use ($cellDatas){
                $sheet->rows($cellDatas);
            });
        })->export('xls');
