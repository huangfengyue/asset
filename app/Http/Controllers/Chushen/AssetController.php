...
class AssetController extends BaseController
{
    ...
    public function lists(Request $request){
        if($request->ajax()){
            ...
            $res = $this->AssetService->getApplyList($pageOption,1,$condition,[],$this->userInfo->role_id);
    ...
    public function jlx(Request $request,$apply_id){
        ...
        if($a["success"]=="false"){
          //  $b=$a["note"].'<script language="javascript"> top.location="/chushen/asset/'.$apply_id.'";</script>';
            return $this->error($a["note"],"/chushen/asset/$apply_id");
            exit;
        }else{
...
//                die();
    ...
    public function updateApply(Request $request,$apply_id){
        if($request->ajax()){
            ...
            if($pass == "ignore"){
                $pass = AssetApply::$STATUS_FIRST_TRIAL_FAIL;
            }elseif ($pass == "back"){
                $pass = AssetApply::$STATUS_MODIFY;
            }else{
                $pass = AssetApply::$STATUS_RETRIAL;
            }
            if($applyInfo = $this->AssetService->getApplyDetailById($apply_id)){
                if(!in_array($applyInfo->status,[AssetApply::$STATUS_WAIT_FIRST_TRIAL])){
                    return $this->responseJson(0,"该单状态异常，无法更改信息");
                }
                if($this->AssetService->applyChushen($apply_id,$pass,$datas,$this->userInfo->user_id)){
                    return $this->responseJson(200,"保存成功");
                }else{
    ...
    public function list_excel(Request $request){
        ...
        $a=array(3,9,15,16,41,42,43,45);
        if(in_array($this->userInfo->user_id,$a)){
            $condition = [
            ];
        }
        ...
        Excel::create('初审审件列表',function($excel) use ($cellDatas){
    ...
    public function ck2_excel(Request $request){
        ...
        Excel::create('初审面签列表',function($excel) use ($cellDatas){
            $excel->sheet('score', function($sheet) use ($cellDatas){
                $sheet->rows($cellDatas);
            });
        })->export('xls');
    ...
    public function repay_excel(Request $request){
        ...
        Excel::create('初审还款列表',function($excel) use ($cellDatas){
            $excel->sheet('score', function($sheet) use ($cellDatas){
                $sheet->rows($cellDatas);
            });
        })->export('xls');
